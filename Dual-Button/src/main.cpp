...
const uint8_t BTNA = 1;
const uint8_t BTNB = 2;
...
void loop() {
	currentValue1 = digitalRead(BTNA);
	currentValue2 = digitalRead(BTNB);
	static int8_t screen = 0;
	static uint8_t swap = 0;
	if (currentValue1 == 0 && lastValue1 == 1) {
		screen += 1;
	}
	lastValue1 = currentValue1;
	if (currentValue2 == 0 && lastValue2 == 1) {
		screen -= 1;
	}
